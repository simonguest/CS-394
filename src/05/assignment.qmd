...
Use Wllama (CPU/WASM) or WebLLM (WebGPU) to run a GGUF model directly in the browser.

**Requirements:**

- Create a web page that loads and runs a GGUF model client-side (no server-side inference)
...
**Deliverable:** A GitHub repository containing your web project with a README that includes which library you chose (Wllama or WebLLM) and a screenshot showing the app running in the browser.
...
- **Option 4**: Wllama is easier to set up (no WebGPU required), but WebLLM offers better performance if your browser supports it. Check [caniuse.com/webgpu](https://caniuse.com/webgpu){.external target="_blank"} for browser support.
