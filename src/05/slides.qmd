...
- Understood the fundamentals and history of diffusion models
- Explored and used models that demonstrate text-to-image, image-to-image, inpainting, outpainting, and ControlNet
- Set up and used Replicate to create a custom pipeline of production-grade models
...
- Rough throughput calculations
  - You have an NVIDIA 3090 (advertised at 35 TFLOPS)
...
- **Consumer**: RX7000 series offer substantial VRAM (up to 24GB) at competitive prices compared to NVIDIA RTX
...
- Unified memory by default, up to 128GB on laptops and 512GB for the Mac Studio with M4 Ultra
...
- Higher memory availability, but lower memory bandwidth
  - ~275GB/s for Spark/MLX; ~1TB/s for 5080; ~1.7TB/s for 5090; ~3TB/s for H100
...
- **Intel**: Acquired Movidius in 2016; released Myriad X in 2017 and a neural compute stick. Superseded by NPUs in Core Ultra processors.
...
- Memory Challenges
  - Consumer-grade GPUs and NPUs have significantly less memory than datacenter-class GPUs
...
- Less memory = only smaller models can run
  - Roughly speaking, size of model must be smaller than available VRAM
...
- Model Architecture
  ...
  - Initial routing layer (also called gating layer) is trained to route tokens to the expert best suited to decode
  - The experts are "activated" for each token
  - Hence, 30B model with 3B active
  - Each token can activate a different number of experts, with attention to previous tokens playing a part

## How MoE Works

- Different use cases
  - Code question routed to experts trained on code
  - Translation question routed to experts trained on language
  - Math question routed to experts trained on logic
...
- Why do this?
  - Faster Inference (token/second): 
    - Because you are only using a subset of experts for each token.
...
Try different prompts. Which experts get activated?
...
- GPTQ (GPT Quantization):
  ...
  - First widely adopted method for aggressive (4-bit) quantization
...
- Q2_K = Aggressive quantization, smallest files, noticeable loss
...
Calling the model from a local notebook
...
(Alternatively use llama.cpp or Ollama from the CLI)
...
{{< embed notebooks/quantization.ipynb#build-llama echo=true outputs=false >}}

## How to Quantize a Model

{{< embed notebooks/quantization.ipynb#convert echo=true outputs=false >}}

## How to Quantize a Model

{{< embed notebooks/quantization.ipynb#convert echo=false >}}

# Calling llama.cpp from Your App
...
- WebLLM ([https://webllm.mlc.ai/](https://webllm.mlc.ai/){.external target="_blank"})
  ...
  - Popular pre-compiled models available - or compile your own
...
const appConfig: webllm.AppConfig = {
  model_list: [
    {
      model: `https://huggingface.co/simonguest/Qwen3-0.6B-it-code-hint-3-MLC`,
      overrides: {
        temperature: 0.5,
      },
    },
...
- Transformers.js [https://huggingface.co/docs/transformers.js](https://huggingface.co/docs/transformers.js){.external target="_blank"}
  - Functionally equivalent to HF's transformer's library, but designed to run in-browser
  - Uses ONNX format instead of GGUF or MLC
  - Latest version (v4) announced Feb 2025, with better WebGPU support
